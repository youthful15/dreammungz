  ...
  return (
    <div className="w-full h-full">
      <div className="flex flex-wrap justify-center w-full h-full p-2 space-x-4 space-y-2 overflow-scroll scrollbar-hide">
        ...
        {info.map(({ type, title }) => {
          return (
            <div className="border-4  w-[340px] h-[45px] text-center rounded-full leading-[42px] mapleStory shadow-md flex bg-beige-500 border-white ">
              <div
                className={`w-[70px]  rounded-l-full ${
                  type === "단편" && " bg-lime-400"
                } ${type === "중편" && " bg-purple-300"} ${
                  type === "장편" && " bg-orange-300"
                }`}
              >
                {type}
              </div>
              <div className="text-center w-[250px]">{title}</div>
            </div>
          )
        })}
